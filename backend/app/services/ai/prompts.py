...
- next_question_index는 이번에 할 질문이 질문 리스트의 몇 번째 항목인지 나타내는 0-based 인덱스(예: [index: 0]이면 0)다. 응답자가 아직 답변을 다 안 했으면 현재 인덱스를 유지하고, 현재 질문에 대해 충분히 답했거나 이미 다음 질문의 내용까지 답변했다면 반드시 다음 인덱스로(+1) 넘어간다. 절대 과거의 인덱스로 되돌아가지 않는다."""
