...
export default function App() {
  ...
  // 2. WebSocket Connection: Connect once per sessionId
  useEffect(() => {
    if (!sessionId) return;
    ...
    function connectWs() {
      ...
      socket.onclose = () => {
        ...
        // 의도적 종료가 아니고 세션이 종료되지 않았으면 3초 후 안전 재연결
        ...
        <main className="shell">
          <section className="glass-panel">
            ...
            <div className="consent-scroll-box">
              ...
              <h3>1. 조사 소개</h3>
              <p>
                귀하는 본 조사의 목적에 적합하여 초대되었습니다. AI 모더레이터와의 대화로 진행되며, 실시간 AI 동시통역을 통해 조사를 의뢰한 클라이언트 측에도 대화 내용이 전달됩니다.
              </p>

              <h3>2. 인터뷰 관찰 및 열람 주체</h3>
              <ul>
                <li><strong>AI 모더레이터</strong>가 실시간 진행을 담당합니다.</li>
                <li><strong>리서치팀(내부 조사 인력)</strong>과 조사를 의뢰한 <strong>클라이언트 측 담당자(PM)</strong>가 별도 백룸에서 실시간 참관할 수 있습니다.</li>
                <li>클라이언트 담당자는 AI 동시통역을 통해 대화를 확인하며, 진행 중 참고할 질문을 리서치팀에 전달할 수 있습니다.</li>
                <li>녹음·녹취본 및 결과 보고서 내 인용구는 식별할 수 없도록 익명화 처리되어 리서치팀과 클라이언트 조사 담당자만 열람하며, 외부에 공유되지 않습니다.</li>
              </ul>

              <h3>3. 수집하는 정보와 이용 목적</h3>
              <ul>
                <li><strong>수집 항목</strong>: 인터뷰 중 답변 내용(음성/텍스트), 녹음·녹취 데이터, 접속 로그 및 디바이스 정보</li>
                <li><strong>이용 목적</strong>: 시장조사 분석 및 클라이언트 보고 목적으로만 사용됩니다.</li>
                <li><strong>보관 기간 및 파기</strong>: 수집된 데이터(음성·텍스트·녹취록)는 조사 완료 및 최종 보고서 납품 완료 시점으로부터 3개월 간 프로젝트 사후 검증 및 품질 관리를 위해 보관되며, 이후 지체 없이 복구 불가능한 방법으로 파기합니다. (단, 관계 법령 및 계약상 별도 보관 의무가 있는 경우는 예외)</li>
                <li><strong>AI 학습 활용 금지</strong>: 수집된 모든 데이터는 엄격한 보안 프로토콜 하에 관리되며, 당사 또는 제3자의 AI 모델 학습(훈련) 목적으로 절대 사용되지 않습니다.</li>
              </ul>

              <h3>4. 시장조사 윤리 및 데이터 보안 정책</h3>
              <ul>
                <li><strong>글로벌 리서치 표준 준수</strong>: 본 조사는 ESOMAR(세계조사회협회) 및 국내외 개인정보보호 법령(PIPA/GDPR 등)의 윤리 가이드라인을 준수합니다. 보고서에 인용되는 모든 응답은 개인을 식별할 수 없는 비식별화(가명·익명) 과정을 거칩니다.</li>
                <li><strong>안전한 데이터 보안 및 관리</strong>: 인터뷰 중 처리되는 오디오 스트림 및 대화 데이터는 엔터프라이즈 클라우드 보안 기준에 따라 전송 및 보관 전 과정에서 안전하게 보호·관리되며, 인가되지 않은 접근을 엄격히 차단합니다.</li>
                <li><strong>답변 시 유의사항</strong>: 인터뷰 중 주민등록번호, 계좌번호, 비밀번호 등 개인 식별이 가능한 민감 정보는 언급하지 말아 주세요. 응답자가 자발적으로 언급한 민감 정보에 대해서는 당사가 책임지지 않습니다.</li>
              </ul>

              <h3>5. 참여자의 권리</h3>
              <p>
                인터뷰 도중 언제든 답변을 거부하거나 참여를 중단할 수 있습니다. 중단하셔도 어떠한 불이익도 없습니다. 또한 개인정보 열람, 정정·삭제, 처리 정지를 요구할 수 있습니다.
              </p>

              <h3>6. 문의</h3>
              <p>
                본 조사 및 개인정보 처리, AI 모더레이터 운영과 관련한 문의사항은 contactus@gromit.ai 로 연락 주시기 바랍니다.
              </p>
            </div>
